...
for perc in [0.1, 0.25, 0.5, 0.75, 1]:
    features_train, features_test, labels_train, labels_test = preprocess(max_df = max_df, percentile=perc)
    ...
    # Compare Algorithms
    ax.bar(np.arange(len(names))+perc-0.5, height = results, width = 0.2, label = perc)
